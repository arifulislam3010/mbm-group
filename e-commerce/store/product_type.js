...
export const actions = {
    async GET_DATA ({ commit }, request) {
        ...
        console.log('Product types data request')      
        try {
            const { data } = await this.$axios.get('/api/product-types/load' + (request['limit']>0?'/' + request['limit']:'') + (request['page']?'?page=' + request['page']:''), {
                headers: {
