<template>
    <div v-if="data.length > 0" class="content_view_block">
        <!-- {{ data }} -->
        <div v-for="(element,elm_index) in data" class="block relative element_layer_block" :key="elm_index">
            <div class="grid grid-cols-12 relative grid_cols_block">
                <div :class="['block','col-span-' + ($device.isMobile?12:column.size?column.size:6),'relative','rounded-md']" v-for="(column,col_index) in element.column" :key="col_index">
                    <template v-if="column.data.length > 0">
                        <div :class="['block','relative','content_layer_block']" v-for="(item,index) in column.data" :key="index">
                            <div v-if="item.type=='rich_txtarea' || item.type=='embed'" class="ql-editor" v-html="item.content"></div>
