<template>
    <div class="content_entry_form_block">
        <RainbowLoader v-if="req_submit" />
        <div class="row">
            <div class="col-md-8">
                <div v-if="form_loader" class="row">
                    <div class="col-md-12">
                        <div class="form_block">
                            <FormBlockLoader :cols="1" :height="65" :r1="true" :r2="true" :r1_w="50" :r2_w="100" :r1_h="20" :r2_h="25" />
                            <FormBlockLoader class="mt-2" :cols="1" :height="65" :r1="true" :r2="true" :r1_w="50" :r2_w="100" :r1_h="20" :r2_h="25" />
                            <FormBlockLoader class="mt-2" :cols="1" :height="65" :r1="true" :r2="true" :r1_w="50" :r2_w="100" :r1_h="20" :r2_h="25" />
                        </div>
                    </div>
                </div>
                <div v-else class="form_block">
                    <div class="mb-4">
                        <label>Area Name <sup>*</sup></label>
                        <div><input type="text" placeholder="Enter area name" class="form-control" v-model="formData.title" ref="area_title" /></div>
                    </div>
                    <div class="mb-4">
                        <label>Delivery Charge ({{ $store.state.currency_info.title }})</label>
                        <div><input type="number" placeholder="i.e 100" class="form-control" v-model="formData.delivery_charge" ref="delivery_charge" /></div>
                    </div>
                    <div>
                        <label>Status</label>
                        <SwithcBtn :status="formData.status" :index="'status'" />
                        <input type="hidden" v-model="formData.status" />
                    </div>
                </div>
            </div>
            <div class="col-md-4">
                <div v-if="form_loader" class="row">
                    <div class="col-md-12">
                        <div class="info_block">
                            <FormBlockLoader :cols="1" :height="65" :r1="true" :r2="true" :r1_w="50" :r2_w="100" :r1_h="20" :r2_h="25" />
                            <FormBlockLoader class="mt-2" :cols="1" :height="65" :r1="true" :r2="true" :r1_w="50" :r2_w="100" :r1_h="20" :r2_h="25" />
                            <FormBlockLoader class="mt-2" :cols="1" :height="65" :r1="true" :r2="true" :r1_w="50" :r2_w="100" :r1_h="20" :r2_h="25" />
                        </div>
                    </div>
                </div>
                <div v-else class="info_block">
                    <div class="item_block mb-4">
                        <label>Choose Region <sup>*</sup></label>                        
                        <select v-model="formData.region_id" class="form-control parent-dropdown" @change="load_cities" ref="region_list">
                            <option value="">Select One</option>
                            <template v-for="(item,index) in region_list" >
                                <option :key="index" :value="item.id">{{ item.title }}</option>
                            </template>
                        </select>
                    </div>
                    <div class="item_block mb-4">
                        <label>Choose City <sup>*</sup></label>                        
                        <select v-model="formData.city_id" class="form-control parent-dropdown" ref="city_list">
                            <option value="">Select One</option>
                            <template v-for="(item,index) in city_list" >
                                <option :key="index" :value="item.id">{{ item.title }}</option>
                            </template>
                        </select>
                    </div>
                </div>
            </div>
        </div>
    </div>
</template>
<script>
import SwithcBtn from '@/components/action_buttons/SwitchBtn'
export default {
    name: 'ContentEntryFromBlock',
    props: {
        edit_content_id: Number
    },
    components: {
        SwithcBtn
    },    
    data(){
        return {
            region_list: [],
            city_list: [],
            form_loader: false,
            formData: {
                title: '',
                region_id: '',
                city_id: '',
                delivery_charge: '',
                status: true
            },
            req_submit: false,
            form_action: 'save'
        }
    },
    computed: {
        form_submit_status () {
            return this.$store.state.form_submit_status        
        }
    },
    watch: {
        edit_content_id (val) {
            if(val) this.load_req_data(val);
        },
        form_submit_status (status) {
            if(status) this.formSubmit();        
        }
    },
    async mounted(){
        // load region list        
        if(this.edit_content_id) await this.load_req_data(this.edit_content_id);
        if(this.$store.state.form_submit_status) this.formSubmit();
        
        this.load_regions();
    },
    methods: {
        switch_data(index,status){
            this.formData[index] = status
        },
        async load_cities(){
            let url = '/api/cities?region_id=' + this.formData.region_id;
            
            this.city_list = [];
            this.$axios.get(url, this.$parent.header_config).then( (response) => {
                console.log(response)
                this.city_list = response.data.data
            }).catch(e => {
                this.$toast.error('City load failed!!!', {icon: "error_outline"})
            });
        },
        async load_regions(){            
            let url = '/api/regions';
            
            this.region_list = [];
            this.$axios.get(url, this.$parent.header_config).then( (response) => {
                console.log(response)
                this.region_list = response.data.data
                
                if(this.city_list.length==0 && this.formData.region_id) this.load_cities()
            }).catch(e => {
                this.$toast.error('Region load failed!!!', {icon: "error_outline"})
            });
        },        
        async load_req_data(id){
            this.form_loader = true;
            
            this.$axios.get('/api/areas/edit/' + id, this.$parent.header_config).then( (response) => {
                console.log('Get Data', response.data)
                let getData = response.data;

                this.formData = {
                    title: getData.title,
                    region_id: getData.region_id,
                    city_id: getData.city_id,
                    status: getData.status
                }

                this.form_loader = false;
                this.form_action = 'update';
            }).catch(e => {
                console.log(e)
                this.$toast.error('Failed!!!', {icon: "error_outline"})
                this.form_loader = false;
            });
        },
        async formSubmit(){
            this.$parent.form_submit_state(false)

            if(this.formData.title.trim()==''){
                this.$toast.error('Please enter area name', {icon: "Warning"});
                this.$refs.area_title.focus();
                return false;
            }else if(this.formData.region_id==''){
                this.$toast.error('Please choose region', {icon: "Warning"});
                this.$refs.region_list.focus();
                return false;
            }else if(this.formData.city_id==''){
                this.$toast.error('Please choose city', {icon: "Warning"});
                this.$refs.city_list.focus();
                return false;
            }

            if(confirm('Are you sure to submit it?')){
                // setup submitted data
                let submit_data = {
                    id: this.$parent.user_id,
                    access_token: this.$parent.user_access_token,
                    data: this.formData,
                    action: this.form_action,
                    edit_id: this.edit_content_id
                }

                // call for submit
                this.req_submit = true;
                
                await this.$parent.areaSubmit(submit_data);
                
                this.req_submit = false;
                this.$parent.load_data();                
                
                await this.$swal("Good job!", "Data has been "+ (this.form_action == 'save'?'inserted':'updated') +" successfully.", "success");

                if(this.form_action == 'save'){
                    this.form_reset();
                    this.$parent.add_new_entity(false)
                }
            }            
        },
        form_reset(){
            this.formData = {
                title: '',
                region_id: '',
                city_id: '',
                delivery_charge: '',
                status: true
            }
        }
    }
}
</script>
<style scoped>
    .form_block,.info_block{
        display: block;
        background-color: #fff;
        border: 1px solid #ddd;
        padding: 20px;
    }
</style>