            ...
            <tbody>
                <template v-if="data.length>0">
                    <tr v-for="(item,index) in data" :key="index">
                        ...
                        <td>{{ item.title }}</td>
                        <td>{{ item.region_info.title }}</td>
                        <td>{{ item.city_info.title }}</td>
                        <td>
                            <div v-if="item.delivery_charge">{{ $store.state.currency_info.title }} {{ item.delivery_charge }}</div>
    ...
    methods: {
        ...
        del_item: function(id){
            if(confirm('Are you sure to delete it?')){
                this.$axios.get('/api/areas/delete/' + id, this.$parent.header_config).then( (response) => {
