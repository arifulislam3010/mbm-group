        ...
        <div class="row">
            ...
            <div class="col-md-4">
                ...
                <div v-else class="info_block">
                    ...
                    <div class="mb-4">
                        <label>Colors <em class="ml-3">[<small class="all_color_sel_btn ml-2 mr-2" @click="$refs.color_manage.sel_all_color()">Select All</small>]</em></label>
                        <ColorsManagement :color_list="$parent.color_list" ref="color_manage" />
                    </div>
                    <div class="mb-4">
                        <label>Tags</label>
                        <TagsManagement />
...
export default {
    ...
    mounted(){
        ...
        // $(".category_list_block").scrollTop($(".category_list_block").find(".item_block.active").offset().top);
    },
    methods: {
        ...
        replace_update_sort_list(type,data){
            if(type=='photo') this.formData.product_photo_infos = data
        },
        ...
        async load_req_data(id){
            ...
            this.$axios.get('/api/products/edit/' + id, this.$parent.header_config).then( (response) => {
                 ...
                 * Product type infos
                 */
                 let getProductTypeInfos = [], get_product_type_ids = {};
                if(getData.product_type_info){
                  getData.product_type_info.forEach( async (v,i) => {
                      ...
                      get_product_type_ids[v.product_type_id] = true;
                ...
                getData.product_price_infos.forEach((v,i) => {
                    let obj = {
                        ...
                        photo_infos: [],
                        remarks: v.remarks,
                        status: v.status?v.status:false
                    }

                    v.product_price_contents.map(vc => {
                        if(get_product_type_ids[vc.product_type_id]){
                            let productTypeInfo = {
                                index: 0,
                 ...
                 *  Selected Tags
                 */
                let getTags = [], tagList = [], ti = 0;
                getData.tag_info.forEach((v,i) => {
                    if(v.tags_data_info){
                        getTags[ti] = {};
                        getTags[ti]['tiClasses'] = [];
                        getTags[ti]['text'] = v.tags_data_info.tag_title
                        getTags[ti]['tiClasses'][0] = 'ti-valid'
