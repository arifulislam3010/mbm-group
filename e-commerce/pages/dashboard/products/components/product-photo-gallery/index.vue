<template>
    <div class="product_photo_gallery_block">
        <label>Photos ({{ photo_arr.length }}) <em class="mr-2"><small><i class="fa fa-up-down-left-right"></i> Sort</small></em> <SwithcBtn :status="drag_n_drop_mode" :index="'drag_n_drop_mode'" :opt1="'Disable'" :opt2="'Enable'" />
            <input type="hidden" v-model="drag_n_drop_mode" /></label>
        <div class="info_form_block mt-1">
            ...
            <div v-else>
                <div class="info_list">
                    <div class="sorting_photo_list_block" v-if="drag_n_drop_mode">
                        <draggable v-model="photo_arr" handle=".photo_block" group="photo_list_setup" @start="drag=true" @end="drag=false" @change="reorder_photo_list()">
                            <div v-for="(item,index) in photo_arr" :key="index" class="photo_block photo_item_block">
                                <div class="img">
...
export default {
    ...
    data(){
        return {
            photo_arr: this.product_photo_infos,
            drag_n_drop_mode: false,
            drag: false
        }
    },
    watch: {
        product_photo_infos: function(data){
            this.photo_arr = data
        }
    },
    methods: {
        reorder_photo_list: function(){
            // this.$parent.formData.product_photo_infos = this.photo_arr
            this.$parent.replace_update_sort_list('photo',this.photo_arr)
        },
        ...
        switch_data(index,status){
            this.drag_n_drop_mode = status
        }
    }
}
</script>
<style lang="scss" scoped>
    .product_photo_gallery_block label{
        display: flex;
        align-items: center;
