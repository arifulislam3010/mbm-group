<template>
    <div class="product_size_info_block_container">
...
export default {
    name: 'ProductSizeInfoBlockContainer',    
    components: {
        ContentEntryForm,
        ContentList
    },
    ...
    methods: {
        ...mapActions({        
            sizeSubmit: 'product_size/DATA_SUBMIT'
        }),
        ...mapMutations({
        ...
        async load_data(pg=this.cur_page){
            let url = '/api/product-size-info'+ (this.limit>0?'?limit=' + this.limit:'') + (pg>1?'&page=' + pg:'') + (this.$parent.$parent.role_access.view_others=='0'?'&own_result=true':'') + (this.$store.state.search_keyword?'&keyword=' + this.$store.state.search_keyword:'');
