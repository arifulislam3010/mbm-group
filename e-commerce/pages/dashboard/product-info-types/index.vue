<template>
    <div class="product_info_types_block_container">
...
export default {
    name: 'ProductInfoTypesBlockContainer',
    components: {
        ContentEntryForm,
        ContentList
    },
    ...
    methods: {
        ...mapActions({
            productInfoTypeSubmit: 'product_info_type/DATA_SUBMIT'
        }),
        ...mapMutations({
        ...
        async load_data(pg=this.cur_page){
            let url = '/api/product-info-types'+ (this.limit>0?'?limit=' + this.limit:'') + (pg>1?'&page=' + pg:'') + (this.$parent.$parent.role_access.view_others=='0'?'&own_result=true':'') + (this.$store.state.search_keyword?'&keyword=' + this.$store.state.search_keyword:'');
