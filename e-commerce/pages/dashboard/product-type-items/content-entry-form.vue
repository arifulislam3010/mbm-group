            ...
            <div class="col-md-8">
                ...
                <div v-else class="form_block">
                    <div class="mb-4">
                        ...
                        <div><input type="text" placeholder="Enter type name" class="form-control" v-model="formData.type_name" ref="type_name" /></div>
                    </div>                    
                    <div>
                        <label>Status</label>
            ...
            <div class="col-md-4">
                ...
                <div v-else class="info_block">
                    <div class="item_block">
                        ...
                        <select v-model="formData.product_type_id" class="form-control parent-dropdown" size="3" ref="product_type">
    ...
    methods: {
        ...
        async load_product_types(){            
            ...
            this.$axios.get(url, this.$parent.header_config).then( (response) => {
                ...
                this.$toast.error('Product types load failed!!!', {icon: "error_outline"})
            });
        },
        async load_req_data(id){
            this.form_loader = true;
            
            this.$axios.get('/api/product-type-info-name/edit/' + id, this.$parent.header_config).then( (response) => {
                console.log('Get Data', response.data)
                let getData = response.data;

                this.formData = {
                    type_name: getData.type_name,
                    product_type_id: getData.product_type_id,
        ...
        async formSubmit(){
            ...
            if(this.formData.type_name.trim()==''){
                this.$toast.error('Please enter tag name', {icon: "Warning"});
                this.$refs.type_name.focus();
                return false;
            }else if(this.formData.product_type_id==''){
                this.$toast.error('Please choose product type', {icon: "Warning"});
            ...
            if(confirm('Are you sure to submit it?')){
                ...
                await this.$parent.productTypeNameSubmit(submit_data);
