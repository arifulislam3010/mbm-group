<template>
    <div class="product_info_by_product_type">
        <label>
            <span>Product type wise products</span>
        ...
        <div v-else-if="contents.length>0">
            <!-- <GChart
                ...
                /> -->
            <PieChart :chartOptions="canvasChartOptions" :content_type="'ProductType'" ref="pie_chart" />
        </div>
...
export default {
    name: 'ProductInfoByProductTypeBlock',
    components: {
        PieChart
    },
    data(){
        return {
            ...
            chartOptions: {
                chart: {
                    title: 'Total products by product type',
                    subtitle: 'Product type and total post',                    
                },
                // is3D: true,
                // pieHole: 0.4,
                slices: {},
    ...
    methods: {
        async load_data(){
            let url = '/api/report-data/products-by-product-type';
            ...
            this.$axios.get(url, this.$parent.header_config).then( (response) => {
                ...
                if(response.data.data.length>0){
                    ...
                    this.chartData[i++] = ['Product Type','Total Post'];
                    ...
                    this.contents.forEach(v => {                        
                        ...
                        this.chartData[i++] = [v.product_type.type_title,get_total_post];
                        ...
                        let obj = {
                            ...
                            "label": v.product_type.type_title,
                            "name": v.product_type.type_title,
                            // "color": "#F7B731",
                            // "legendLabel": "(+ 1°F < Temp <= 2°F)"
                        }
...
<style scoped>
    ...
    .product_info_by_product_type{
        display: block;
    }
    .product_info_by_product_type > div{
        display: block;
        min-height: 200px;
    ...
    .product_info_by_product_type > div.loader_block,
    .product_info_by_product_type > div.empty_block{
        text-align: center; padding: 35px 0; color: #999;
    }
    .product_info_by_product_type > div.loader_block > i,
    .product_info_by_product_type > div.empty_block > i{
