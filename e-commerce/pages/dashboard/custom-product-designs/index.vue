<template>
    <div class="custom_product_designs_block_container">
...
export default {
    name: 'CustomProductDesignsBlockContainer',    
    components: {
        ContentEntryForm,
        ContentList
    },
    ...
    methods: {
        ...mapActions({        
            customProductDesignSubmit: 'custom_product_design_info/DATA_SUBMIT'
        }),
        ...mapMutations({
        ...
        async load_data(pg=this.cur_page){
            let url = '/api/custom-product-designs'+ (this.limit>0?'?limit=' + this.limit:'') + (pg>1?'&page=' + pg:'') + (this.$parent.$parent.role_access.view_others=='0'?'&own_result=true':'') + (this.$store.state.search_keyword?'&keyword=' + this.$store.state.search_keyword:'');
