<template>
    <div class="product_types_block_container">
...
export default {
    name: 'ProductTypesBlockContainer',
    components: {
        ContentEntryForm,
        ContentList
    },
    ...
    methods: {
        ...mapActions({
            productTypeSubmit: 'product_type/DATA_SUBMIT'
        }),
        ...mapMutations({
        ...
        async load_data(pg=this.cur_page){
            let url = '/api/product-types'+ (this.limit>0?'?limit=' + this.limit:'') + (pg>1?'&page=' + pg:'') + (this.$parent.$parent.role_access.view_others=='0'?'&own_result=true':'') + (this.$store.state.search_keyword?'&keyword=' + this.$store.state.search_keyword:'');
