        ...
        <table v-else class="table table-striped">
            <thead>
                <tr>
                    ...
                    <th>Icon</th>
                    <th>Category</th>
                    <!-- <th style="text-align:center" width="80">Open</th> -->
            ...
            <tbody>
                <template v-if="data.length>0">
                    <tr v-for="(item,index) in data" :key="index">
                        ...
                        <td>{{ item.cat_info?item.cat_info.category_name:'For All' }}</td>
                        <!-- <td align="center">
                            <span class="open_entry">
                                <i v-if="item.open_entry" class="fa fa-lock-open active"></i>
    ...
    methods: {
        ...
        del_item: function(id){
            if(confirm('Are you sure to delete it?')){
                this.$axios.get('/api/product-types/delete/' + id, this.$parent.header_config).then( (response) => {
