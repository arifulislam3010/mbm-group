...
class ReportController extends Controller
{
    ...
    public function completeness(Request $request){
        ...
        if(Request()->map_index=='enrolled'){
            ...
            WHERE ce.course_batch_id = ' . $batch->id . ' AND ce.id>=4187676
            GROUP BY e.DistrictId';

            $cmp = $this->redisCache($qryStr);
            if(empty($cmp)){
                $cmp = DB::select($qryStr);
                $this->redisCache($qryStr, $cmp);
            }
            //$cmp = DB::select($qryStr);
            ...
            $res = $this->redisCache($qryStr);
            if(empty($res)){
                $res = DB::select($qryStr);
                $this->redisCache($qryStr, $res);
            }
            //$res = DB::select($qryStr);
    ...
    public function donut(Request $request)
    {
        ...
        $totalEnrolled = $this->redisCache($batch->id.'totalEnrolled');
        if(empty($totalEnrolled)){
            $totalEnrolled = CourseEnrollment::where('course_batch_id',$batch->id)->count(DB::raw('DISTINCT order_id'));
            $this->redisCache($batch->id.'totalEnrolled', $totalEnrolled);
        }
        ...
        $totalCertificate = $this->redisCache($batch->id.'totalCertificate');
        if(empty($totalCertificate)){
            $totalCertificate = CourseEnrollment::where('course_enrollments.course_batch_id',$batch->id)
                ->JOIN('certificate_submit AS cs','cs.course_enrollment_id','course_enrollments.id')
                ->where('cs.file_name','!=','')
                ->count();
            $this->redisCache($batch->id.'totalCertificate', $totalCertificate);
        }
        ...
        $division = $this->redisCache($batch->id.'division');
        if(empty($division)){
            $division = DB::table('emis_division')->get();
            $this->redisCache($batch->id.'division', $division);
        }
        //$division = DB::table('emis_division')->get();
        ...
        $totalInfo = $this->redisCache($batch->id.'totalInfo');
        if(empty($totalInfo)){
            ...
            $this->redisCache($batch->id.'totalInfo', $totalInfo);
        }
        ...
        $enrollInfo = $this->redisCache($qryStr);
        if(empty($enrollInfo)){
            $enrollInfo = DB::select($qryStr);
            $this->redisCache($qryStr, $enrollInfo);
        }
        ...
        $qryStr = 'SELECT e.DivisionId AS id,COUNT(DISTINCT ce.order_id) AS total_cer
                ...
                WHERE ce.course_batch_id = ' . $batch->id . '
                AND c.file_name != ""  AND ce.id>=4187676
                GROUP BY e.DivisionId';

        $certificateInfo = $this->redisCache($qryStr);
        if(empty($certificateInfo)){
            $certificateInfo = DB::select($qryStr);
            $this->redisCache($qryStr, $certificateInfo);
        }
        //$certificateInfo = DB::select($qryStr);
    ...
    public function all_data(Request $request){
        ...
        if(empty($enrollInfo)){
            ...
            $dis = $this->redisCache($qryStr);
            if(empty($certificateInfo)){
                $data = DB::select($qryStr);
                $this->redisCache($qryStr, $data);
            }
