...

{
    ...
    public function enrollCourse(Request $request){
        ...
        $batch = RestrictedUser::select('restricted_users.user_type','restricted_users.batch_id','cb.owner_id','cb.uuid')
        ...
        $arr['service']['course']['access']['course']['view_all'] = false;
        $arr['service']['course']['access']['batch']['view_all'] = false;
        $arr['service']['course']['access']['dashboard']['header-stats_all'] = false;
        $arr['service']['course']['access']['people']['delete'] = false;
            ...
            foreach ($value['access'] as $key1 => $value1) {
                ...
                if($batch->user_type==1){
                   ...
                   if($check){
                        ...
                        $userdata['uuid'] = $batch->uuid;
                        $userdata['user_type'] = $batch->user_type;
                        $response = ['data' => $user,'user'=>$userdata];

                       return response()->json($response,200);
                       return response()->json(['message' => 'You are already enrolled in this course.!']);
                   }
                   else{
